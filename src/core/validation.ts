// ponytail: single-file validation helpers
export function checkAudioValid(
  ...
  let clipCount = 0
  for (let i = 0; i < samples.length; i++) {
    if (Math.abs(samples[i]) > 0.999) clipCount++
  }
  if (clipCount / samples.length > 0.01) warnings.push(`audio: high clipping ratio (${((clipCount / samples.length) * 100).toFixed(1)}%)`)

  return warnings
}

export function checkImageValid(
...
export function checkEmbeddingValid(
  values: Float32Array,
  dimensions: number,
): string[] {
  const warnings: string[] = []
